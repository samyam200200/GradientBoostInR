...
#recoding the diagnosis variables - knn
wbcd_xgtest$diagnosis<- factor(wbcd_xgtest$diagnosis, levels = c(0, 1),
                         labels = c("Benign", "Malignant"))
wbcd_xgtrain$diagnosis<- factor(wbcd_xgtrain$diagnosis, levels = c(0, 1),
                               labels = c("Benign", "Malignant"))
#scaling the data for use in knn
scaled_testdata<-as.data.frame(scale(wbcd_xgtest[c(-1, -31)]))
scaled_traindata<-as.data.frame(scale(wbcd_xgtrain[-1]))
#using knn to train the model using k=21 since root of train data set=21
wbcd_xgtest$predknn<-knn(train=scaled_traindata, test=scaled_testdata[-31], cl = wbcd_xgtrain$diagnosis, k=21)
CrossTable(x=wbcd_xgtest$diagnosis, y=wbcd_xgtest$predknn,prop.chisq=FALSE)

#naivebayes
modelnb<-naive_bayes(diagnosis~., data=wbcd_xgtrain)
wbcd_xgtest$prednb<-predict(model, wbcd_xgtest[2:31])
CrossTable(x=wbcd_xgtest$diagnosis, y=wbcd_xgtest$prednb, prop.chisq = F)

#logistic regression
modellr<-glm(diagnosis~., data=wbcd_xgtrain, family="binomial")
prob<-predict(modellr, wbcd_xgtest[2:31], type="response")
wbcd_xgtest$predlr<-ifelse(prob>0.5, 1,0)
CrossTable(x=wbcd_xgtest$diagnosis, y=wbcd_xgtest$predlr, prop.chisq = F)

#random forest
modelrf<-ranger(diagnosis~., wbcd_xgtrain, num.trees = 500, respect.unordered.factors = "order")
wbcd_xgtest$predrf<-predict(modelrf, wbcd_xgtest[2:31])$predictions
CrossTable(x=wbcd_xgtest$diagnosis, y=wbcd_xgtest$predrf, prop.chisq = F)
